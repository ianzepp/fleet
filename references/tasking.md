...
| Slot | Workspace role | Commits? |
| --- | --- | --- |
| **hand-1..hand-N** | Floaters — dynamically assigned by repo/worktree/crate; rehome when reassigned | **Yes** — Hand commits own work on assigned branch; Mind decides branch strategy at assignment time |
...
| **hand-N** just finished a **unit** (not theme) | Not success-idle | Absorb/review; **refill** next **product** map unit (or reassign) |
| **hand-N** idle + empty + ready work exists only in a repo already being written by another Hand | Valid dependency / write-scope pause | Do not stack overlapping work; choose a different ready repo, serialize, or record defer |
| **hand-N** after **theme** on a feature branch, empty, waiting Mind merge decision | **Operational pause** | Review → accept → Mind merges when ready; optional light pivot if map has unrelated **product** work |
...
Do not sleep merely because bags are empty **if** map still has unblocked **product** work (and posture allows). Do sleep when bags empty **and** no honest product next — check map, `pending_reviews`, then posture.
