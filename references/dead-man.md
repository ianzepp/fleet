...
> **Steward implementation is paused.** The dead-man concept remains valid, but
> the `steward.sh` script has been removed. A Vivi-native steward will be
> designed separately. The commands and script references below are retained as
> the design record for that future implementation; no `steward.sh` exists to
> invoke today.
...
**Steward is disabled by default.** Arming a Mind loop / `FLEET_CYCLE` schedule does **not** arm steward. Until the Vivi-native steward lands, steward cannot be armed at all.
...
| A steward implementation exists | `steward.sh` is removed; Vivi-native steward is pending |
...
| **Steward** | watchdog process + loop (implementation pending) |
...
Mind session (operator TUI) — may supervise one or more fleets
  ...
  │     write F's last_successful_cycle_at
  │     (future: rearm Vivi-native steward for F)
...
F/.vivi/mind-baseline.json   (+ optional F/.vivi/steward.rearm touch)
  ▲
  │  poll
watchdog target from the Vivi role record (legacy steward:1.1 or mgs:steward.1)
...
# future (when Vivi-native steward exists and is armed for this fleet):
#   steward.last_rearm_at = now
#   touch $PROJECT/.vivi/steward.rearm
```

**Do not** rearm only on turn start (masks hook death). Optional: rearm at **start and end** of long cycles so grace survives multi-minute acts. Skip all rearm when steward is disabled/unarmed (today: always skip — no implementation exists).

## Arm / disarm

> **Not implemented today.** `steward.sh` was removed; the commands below are
> the design shape for the future Vivi-native steward. Do not call them.

```text
# future, only after operator enabled this fleet AND asked to arm:
steward arm --project <root>
steward disarm --project <root>  # required when stopping if it was armed
```

**Do not arm when:** loop-only / manual cycles; `steward.enabled` false or missing; operator silent on steward; no steward implementation exists (current state).
...
**Policy exception (narrow):** `steward.notify.preauthorized_exec_send: true` for **this template only** — trip page to configured `to`. Not a general agent send license.
...
3. Clear tripped state in baseline (future: `steward clear --project <root>`)
...
Soft-hold: the implementation reads each hand’s `tmux_target` from its Vivi role record (never hardcode session==`hand-1`).
...
## Script (removed — design record)

> The `steward.sh` script has been removed. The command surface below is the
> design record for the future Vivi-native steward. Nothing here is invocable
> today.

```text
steward arm    --project <root>
steward rearm  --project <root>
steward disarm --project <root>
steward status --project <root>
steward check  --project <root>   # one-shot; may trip
steward clear  --project <root>
```
...
| Event | Mind action |
| --- | --- |
| Successful FLEET_CYCLE end | write `last_successful_cycle_at`; rearm only if steward armed **and implemented** |
| Long cycle start (optional) | early rearm only if steward armed **and implemented** |
| Stop loop / wind-down | disarm same turn if it was armed |
| Start loop alone | **do not** arm steward |
| Operator asks steward on **this** fleet | set `enabled: true` if needed, then arm (when implementation exists) |
| Steward missing while armed | recreate via arm (when implementation exists) |
...
- Calling `steward.sh` — the script is removed; await Vivi-native steward
